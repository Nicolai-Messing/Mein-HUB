...
Finanzen sowie freie Projekterstellung lassen sich als weitere Tabellen +
Tabs auf dieselbe Weise ergänzen.

## Update: Aktivität, Umsatzziele, Provision & Organigramm-Status

Falls du das Projekt bereits einmal deployed hattest: Führe zusätzlich
`supabase/schema_update_2.sql` im Supabase SQL Editor aus (legt die Tabellen
`rollen`, `aktivitaet`, `umsatz_monat` an und erweitert `team` um `rolle_id`
und `status`). Danach den neuen Code aus `components/Hub.js` und
`app/layout.js` in dein GitHub-Repository hochladen — Vercel deployt
automatisch neu.

Was sich geändert hat:
- **Aktivität** (vorher "Termine"): kein Eintragen einzelner Termine mehr,
  sondern pro Monat und Teammitglied ein Ziel- und ein IST-Wert für
  Erstgespräche und Termine, mit Balkendiagrammen zur Zielerreichung.
- **Umsatz**: pro Monat nur noch vier Werte — Umsatzziel, IST Neu, IST
  Bestand, Prognose nächste Woche — mit Zielerreichungs-Anzeige.
- **Provisionsrechner**: errechnet automatisch "Mein Verdienst" aus dem
  Gesamtumsatz und den Rollen-Prozentsätzen deiner Struktur (Differenz-
  provision). Rollen & Prozentsätze sind frei editierbar.
- **Organigramm**: Vorlage für 6 direkte Partner mit je 6 weiteren darunter,
  freie Plätze füllen sich nach und nach. Farbcode: Bewerber (hellblau),
  PoPa (dunkles Blau), Aktiver Partner (Gelb), Inaktiver Partner (helles Grau).
